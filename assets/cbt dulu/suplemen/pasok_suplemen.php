<?php 
	...
	if(isset($_POST['pasok'])){
		if(pasokSuplemen($_POST) > 0 ){
			echo "<script>
						alert('berhasil memasok');document.location.href = 'suplemen.php';
					</script>";
		}else{
			echo "<script>
						alert('gagal');document.location.href = 'suplemen.php';
					</script>";
		}
	}
		...
		<div style="width: 550px; height: 660px; background-color: ghostwhite; padding: 30px; border-radius: 10px; margin-left: 280px; margin-top: 40px; ">
			<form method="post">
			  ...
			  <div class="form-group">
			    ...
			    <input type="text" class="form-control" name="nama_suplemen" id="nama_suplemen" value="<?= $suplemen['nama_suplemen']; ?>" readonly>
			  </div>
			  <div class="form-group">
				<label for="pemasok" style="font-size: 20px;">Pemasok</label>
			    <input type="text" class="form-control" name="pemasok" id="pemasok" readonly value="<?= $suplemen['pemasok']; ?>" readonly>
			  </div>
			  <div class="form-group">
